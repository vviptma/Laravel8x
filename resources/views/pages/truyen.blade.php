...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active text-success" aria-current="page">{{$info_truyen->tentruyen}}</li>
        </ol>
    </nav>
    {{--INFOR TRUYỆN--}}
    <div class="card mb-3">
                    ...
                    <div class="card infor">
                        <ul class="list-group list-group-flush">
                            <p class="list-group-item">
                                <i class="fa fa-user"></i><span>Tác giả: <a href="#">{{$info_truyen->tacgia}}</a></span>
                            </p>
                            <p class="list-group-item"><i class="fa fa-folder-open"></i><span>Loại: <a href="{{asset('danhmuc/'.$info_truyen->danhmuctruyen->slug_danhmuc)}}">{{$info_truyen->danhmuctruyen->tendanhmuc}}</a>
                            </span></p>
                            <p class="list-group-item"><i class="fab fa-sourcetree"></i><span>Bản quyền: </span></p>
                            ...
                            <p class="list-group-item"><i class="fas fa-sync-alt"></i><span>{{$info_truyen->updated_at}}</span></p>
                        </ul>
                    </div>
                </div>
                                    ...
                                    @foreach($dsallchapter as $key => $chapter)
                                        @if($key==0)
                                            <a class="btn btn-outline-danger"
                                               href="{{url('chapter/'.$chapter->slug_chapter)}}">Đọc
                                                ngay</a>
                                        @else
                                        @endif
                                    @endforeach
    ...
    <div class="card mb-3">
        <div class="card-header bg-transparent"><h5 class="text-uppercase"><i class="fas fa-list-ol"></i>
            ...
            @if($allchapter > 0)
                @foreach($dsallchapter as $key => $chapter)
                    <a type="button" class="list-group-item list-group-item-action text-dark"
                       href="{{url('chapter/'.$chapter->slug_chapter)}}">{{$chapter->tieude}}</a>
                @endforeach
                <div class="card-footer text-center bg-transparent"><p>{!! $dsallchapter->links() !!}</p></div>
            @else
                <a type="button" class="list-group-item list-group-item-action">Đang được cập nhật thêm...</a>
            @endif
        </div>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div id="fb-root"></div>
            <script async defer crossorigin="anonymous" src="https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v10.0" nonce="w0jhgg7T"></script>
            <div class="fb-comments" data-href="https://webtruyen.com/cuu-tinh-ba-the-quyet/" data-width="100%" data-numposts="5"></div>
        </div>
    </div>
@endsection
@section('sidebar')
